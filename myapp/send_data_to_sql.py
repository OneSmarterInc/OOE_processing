...
def generate_edi_segment_excel(pivot_df_data):
    subfield_mappings = {
    ...
    all_columns = sum(subfield_mappings.values(), []) + ["Date_edi"] + columns_to_keep_blank
    data_to_return = []
        ...
        try:
            ...
            for segment, keys in subfield_mappings.items():
                ...
                if segment == "N4":
                    ...
                    if len(data_list) > 6:
                        itemss = data_list[:-2][2:]
                        new_string = ''.join(i + '_' for i in itemss)
                        data_list = [data_list[0], data_list[1], new_string, data_list[-2], data_list[-1]]
                    result = " ".join(data_list)
                    item[segment] = result
                elif segment == "INS":
                    data = item.get(segment)
            ...
            data_to_return.append(parsed_data)
